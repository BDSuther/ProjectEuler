...
int main(int argc, char *argv[]){
	...
	while(!dMin){
		...
		for(int j=i-1 ; j>0; j--){
			...
			if(pi - pj > d){ //the difference is bigger than the minimum difference already found and so the difference would only get larger the lower we go so break
				break;
			}
		}
		if(pi - plast > d){ //the difference between the current highest number and the one before it is larger than the current min so we cannot find a smaller one
			dMin = true;
		}
	}
